...
namespace Identity.Areas.Identity.Pages.Account.Manage
{
    public class IndexModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync()
        {
            ...
            if (userWithSameEmail != null && userWithSameEmail.Id != user.Id) //check if an Email already exists for another user.
            {
            ...
            if (Input.Email != email)
            {
                ...
                TempData["success"] = "Votre adresse mail a été modifié avec succès!";
            }

            var phoneNumber = await _userManager.GetPhoneNumberAsync(user);
            var fullName = user.FullName;

            // Check if the name entered by the user is already taken
            var existingUser = await _userManager.Users.FirstOrDefaultAsync(u => u.FullName == Input.FullName);
            if (existingUser != null && existingUser.Id != user.Id)
            {
                ModelState.AddModelError(string.Empty, "The name has already been taken.");
            ...
            if (Input.PhoneNumber != phoneNumber)
            {
                ...
                {
                    StatusMessage = "Unexpected error occurred setting phone number.";
            ...
            TempData["success"] = "Votre profil a été modifié avec succès!";
            return RedirectToPage();
        }
    }
}
